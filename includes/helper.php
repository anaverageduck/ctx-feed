...
if ( ! function_exists( 'woo_feed_plugin_installing' ) ) {
    function woo_feed_plugin_installing() {
        ...
        $plugin_version = isset( $_POST['version'] ) ? sanitize_text_field( $_POST['version'] )  : '';

        $result = woo_feed_install_and_activate_plugin($plugin_slug, $plugin_version);
...
function find_main_plugin_file($plugin_dir) {
    ...
    foreach ($files as $file) {
        ...
        if (is_file($path) && pathinfo($path, PATHINFO_EXTENSION) === 'php') {
            ...
            if (preg_match('/^(\s*\/\*\s*|\s*\*\s*)Plugin Name:/mi', $contents)) {
                // Found the main plugin file
                return basename($plugin_dir) . '/' . $file;
            }
        }
    }

    return false; // no main plugin file found
}

if ( ! function_exists( 'woo_feed_install_and_activate_plugin' ) ) {
    function woo_feed_install_and_activate_plugin($plugin_slug, $plugin_version = '')
    {
        ...
        if (!empty($plugin_version) && !isset($api->versions[$plugin_version])) {
          return "failed";
        }

        $plugin_file = find_main_plugin_file(WP_PLUGIN_DIR . '/' . $plugin_slug) ?: "{$plugin_slug}/{$plugin_slug}.php";
        ...
        if (file_exists(WP_PLUGIN_DIR . "/{$plugin_slug}")) {
          ...
          $wp_filesystem->delete(WP_PLUGIN_DIR . "/{$plugin_slug}", true);
        }
        ...
        // Install the plugin
        $downloadLink = !empty($plugin_version) && isset($api->versions[$plugin_version])
          ? $api->versions[$plugin_version]
          : $api->download_link;
